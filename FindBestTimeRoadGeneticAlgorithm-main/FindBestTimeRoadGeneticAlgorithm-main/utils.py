...
def readJson(filename):
    ...
    for i in jsons:
        genePool.append(
            destination.Destination(i["spotName"], re.sub("[^0-9]", "", i["afterPrice"]), i["category"], i["duration"]))
    return genePool
...
def readJsonV2(data):
    ...
    for i in data.iterrows():
        genePool.append((destination.Destination(i[1].state, i[1].users_count, i[1].spot_categories,
                                                 (i[1].radius_meters * 3.14 * 2) / 2, i[1].spot_categories,
                                                 i[1].placeid)))
    return genePool
    ...
    for i in range(len(population)):
        ...
        for j in range(len(population[i][0].genes) - 1):
            if j <= (len(population[i][0].genes) - 1):
                ...
                tDuration = getDuration(durationList, population[i][0].genes[j].name,
                                        population[i][0].genes[j + 1].name)
        ...
        costScore = abs((budget * 10 - ((population[i][0].sumCost() * noPerson) + travelExpenses))) + 100
        # print(costScore)
        durationScore = abs((population[i][0].sumDuration() + travelDuration) - duration) + 100
        population[i][1] = 1 / (costScore + durationScore)
        # total cost
        population[i][2] = (population[i][0].sumCost() * noPerson) + travelExpenses
        # total duration
        population[i][3] = (population[i][0].sumDuration() + travelDuration)
        # travel expenses
        population[i][4] = travelExpenses
        # travel duration
        population[i][5] = travelDuration
...
def showResults(results, noPerson, distanceList, time, typename, durationList):
    with open('newresults.txt', 'a') as outfile:
        ...
        for i in results:
            ...
            totaldist, places_output, routeList, daySuggestion = RouteOptimizer(i, distanceList,
                                                                                durationList).get_optimized_route()
